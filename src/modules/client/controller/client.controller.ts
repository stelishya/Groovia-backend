...
export class ClientController {
  ...
  async uploadProfilePicture(
    ...
    try {
      const result = await this._clientService.uploadProfilePicture(
        userId,
        file,
      );
  ...
  async updateProfile(
    ...
    const updatedUser = await this._clientService.updateClientProfile(
      userId,
      updateData,
    );
  ...
  @HttpCode(HttpStatus.OK)
  async getDancerProfile(@Param('id') dancerId: string) {
    const dancer = await this._clientService.getDancerProfile(dancerId);
    return ApiResponse.success({
      message: MESSAGES.DANCER_PROFILE_RETRIEVED_SUCCESSFULLY,
      dancer,
    });
  }
  ...
  async getAllDancers(
    ...
    const limit = 6;
    const { dancers, total } = await this._clientService.getAllDancers({
      location,
      sortBy,
      page: pageNumber,
      limit,
    ...
    return ApiResponse.success({
      message: MESSAGES.DANCERS_RETRIEVED_SUCCESSFULLY,
      dancers,
      total,
      page,
      limit,
  ...
  async createEventRequest(
    ...
    const newRequest = await this._clientService.createEventRequest(
      createRequestDto,
      clientId,
    );
    // return { message: 'Request sent successfully', request: newRequest };
    return ApiResponse.success({
      message: MESSAGES.REQUEST_SENT_SUCCESSFULLY,
      request: newRequest,
    });
  }
  ...
  async getEventRequests(
    ...
    const { requests, total } = await this._clientService.getEventRequests(
      clientId,
  ...
  async updateEventRequestStatus(
    ...
    const updatedRequest = await this._clientService.updateEventRequestStatus(
      eventId,
      statusDto,
    );
    return ApiResponse.success({
      message: MESSAGES.BOOKING_STATUS_UPDATED_SUCCESSFULLY,
  ...
  async createEventBookingPayment(
    ...
    const paymentOrder = await this._clientService.createEventBookingPayment(
      eventId,
      userId,
    );
    return ApiResponse.success({
  ...
  async verifyEventBookingPayment(
    ...
    const updatedEvent = await this._clientService.verifyEventBookingPayment(
      eventId,
      paymentDetails,
    );
    return ApiResponse.success({
