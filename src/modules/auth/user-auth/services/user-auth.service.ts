...
import type { IUserService } from 'src/modules/users/interfaces/user.service.interface';
...
import {
  type IMailService,
  IMailServiceToken,
} from 'src/modules/mail/interfaces/mail.interface';
import {
  SignupDto,
  SignupResponse,
...
export class UserAuthService implements IUserAuthService {
  ...
  async login(
    ...
    try {
      ...
      const { accessToken, refreshToken } =
        await this._commonService.generateToken(user);
  ...
  async signup(
    ...
    try {
      const { username, email, role, password, confirmPassword, otp } =
        signupDto;
      if (otp) {
        ...
        console.log(
          `[Signup Step 2] OTP verified for ${email}. Attempting to create user.`,
        );
        try {
          const hashedPassword =
            await this._hashingService.hashPassword(password);
          ...
          console.error(
            `[Signup Step 2] User creation FAILED for ${email}`,
            creationError.stack,
          );
          throw new HttpException(
            'Failed to create user after verification.',
      ...
      this._logger.error(
        `Signup process failed for ${signupDto.email}: ${error.message}`,
      );
      console.log(
        'An internal error occured while signup.',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
  ...
  async forgotPassword(
    ...
    if (!user) {
      ...
      return {
        success: false,
        message: `Password reset attempt for non-existent email: ${email} has failed`,
    ...
    this._logger.log(
      `Update result for ${user.email}: ${JSON.stringify(updateResult)}`,
    );
  ...
  async resetPassword(token: string, newPassword: string): Promise<boolean> {
    try {
      ...
      const hashedPassword =
        await this._hashingService.hashPassword(newPassword);
      console.log(
        'hashedPassword in resetPassword in user-auth.service.ts :',
        hashedPassword,
      );

      const passwordUpdated = await this._userService.updatePassword(
        user._id,
        hashedPassword,
      );
      console.log(
        'passwordUpdated in resetPassword in user-auth.service.ts',
        passwordUpdated,
      );
      ...
      this._logger.log(
        `Password has been reset and token cleared for user: ${user.email}`,
      );
      return true;
    } catch (error) {
