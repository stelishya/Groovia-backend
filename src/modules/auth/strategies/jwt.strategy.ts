...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
  ...
  async validate(payload: TokenPayload): Promise<{ userId: string; email: string; username?: string; role?: string[] }> {
    ...
    if (payload.role && (payload.role.includes(Role.ADMIN) || payload.role.includes('admin'))) {
      user = await this._adminService.findOne({ _id: new Types.ObjectId(payload.userId) });
      isAdmin = true;
    } else {
    ...
    const result = {
      ...
      email: user.email,
      username: (user as any).username || (user as any).name, // Admin might not have username same way
      role: isAdmin ? [Role.ADMIN] : ((user as any).role || []),
    };
